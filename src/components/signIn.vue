    ...
    <div class="container_box">
      ...
      >
        ...
        <el-row class="flexbt">
          ...
          <el-button @click="reguser">验证权限</el-button>
        </el-row>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    signInTop: function () {
      ...
      signInObj.validate((result) => {
        if (result) {
          ...
          axios.post("/api/signInadd", this.formData).then((res) => {
    ...
    //验证权限弹框函数
    reguser: function () {
        if(this.$data.signreg){
      this.$prompt("请输入开发者私密获得注册权限、无权限可使用test账号登录->账号：test 密码：test", "提示", {
        confirmButtonText: "确定",
      })
        .then(({ value }) => {
          if (value == "999926") {
            this.$data.signreg = false;
            this.$message({
              type: "success",
              message: "获取权限成功！",
            });
